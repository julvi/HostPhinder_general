...
# For multiple fasta entries in 1 file use the next awk command
#print previous seqlen (if exists)
#print tag
#init seqlen counter, accumulate totals
#awk '/^>/ {if (seqlen){print seqlen}; print ;seqlen=0;next; } { seqlen = seqlen +length($0)}END{print seqlen}' $file
