...
#----------------------------------------------------------------------------------------------------------------------------------
			...
			if ( $i >= 1 ) {
				...
				for my $i ( 0 .. 9 ) {
					if ( defined ( $sortedAoA_Q[$i][10] ) ) {
						push ( @first_ten, $1 ) if $sortedAoA_Q[$i][10] =~ m/"(.*?)"/;
						#push ( @first_tenAA, $sortedAoA_Q[$i] ) if $sortedAoA_Q[$i][10] =~ m/"(.*?)"/;
					}
					
				}
				print Dumper \@first_ten;

				#
				my $first = mode (\@first_ten);
				#my @majority_host;
				my @majority_hostQ;
				for my $i ( 0 .. $#first_ten ) {
					#push ( @majority_host, $sortedAoA_Q[$i] ) if $first_ten[$i] eq $first;
					push ( @majority_hostQ, $sortedAoA_Q[$i][5] ) if $first_ten[$i] eq $first;

				}
				#print Dumper \@majority_host;
				print Dumper \@majority_hostQ;
				my $average = average ( \@majority_hostQ );
				print "$average\n";
				#print "***$first\n";
