...
output_1="$output/$(echo $input | perl -ne '/\/.*\/(.*)\/(.*)$/; print "$1_$2"')_pred"
#output_1="$output/${input#*/}""_pred"
...
if [ $taxonomy = genus ]
	then
		...
		then
		   ...
		   echo -e "$acc\t$pred\t$value" >> ${output}_first/values
		else
		   echo -e "$acc\tNo_significant_match_found" >> ${output}_first/values
