...
output_1="$output/$(echo $input | perl -ne '/\/.*\/(.*)\/(.*)$/;\
 print "$1_$2"')_${taxonomy}_pred"

#python2.7 scripts/findtemplate_scipy.py -t $database -p -k 15 -o $output_1 -i $input -e $evalue
# cge-s2:
/tools/opt/anaconda/bin/python $work_dir/scripts/findtemplate_scipy.py\
 -t $database -p -k 15 -o $output_1 -i $input -e $evalue
...
if [ $taxonomy = species ]
	then
		...
		# cge-s2
		/tools/bin/python2.7 $work_dir/scripts/get_host_alpha.py\
		 -a 2.004 -f $sort_output >> $valdir/sp_alpha_values #${output}_sp_alpha/values
fi
